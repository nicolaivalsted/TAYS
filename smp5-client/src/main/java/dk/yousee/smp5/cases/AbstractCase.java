	...
	public String errorMessageHandler(String original, String detailError) {
		try {
			...
			NodeList nodeList2 = document
					.getElementsByTagName("smp:errorMessage");
			...
			for (int i = 0; i < codelist.size(); i++) {
				original = original + "\n" + "errorcode = " + codelist.get(i)
						+ "  errorMessage = " + messagelist.get(i);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void readSubscriber() {
		try {
			...
			throw new SystemException(String.format(
					"readSubscriber failed, got exception: %s%s",
					e.getMessage(), cause.toString()));
		}
		buildSubscriberModel();
	}
