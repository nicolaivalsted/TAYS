...
/**
 * User: aka Date: 07/09/12 Time: 16.07 One row of data in Pro Store.
 */
...
//    }
            ...
            if (PRODUCT_ID_YSPRO.equals(one.getKey())) {
                ...
                if (one.getValue().isJsonArray()) {
                    for (JsonElement e : one.getValue().getAsJsonArray()) { //hax for perSon jSon
                        ...
                        properties.put(o.get("DataName").getAsString(), o.get("Value").getAsString());
                    }
                }
            } else {
                if (!one.getValue().isJsonNull())
    ...
    public JsonElement printJson() {
        ...
        if (getPos() != null)
            res.addProperty(POS_KEY, getPos());
        if (getProduct() != null)
            res.addProperty(PRODUCT_ID_YSPRO, getProduct().toString());
        if (getUuid() != null)
            res.addProperty(UUID_KEY, getUuid());
        if (getFrom() != null)
            res.addProperty(FROM_DATE_KEY, getFrom().toString());
        if (getTo() != null)
            res.addProperty(TO_DATE_KEY, getTo().toString());
        if (getStatus() != null)
            res.addProperty(STATUS_KEY, getStatus());
        if (getOrderForSame() != null)
            res.addProperty(ORDER_4_SAME, getOrderForSame());
