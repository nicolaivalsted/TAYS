...
public class YsProApiIT {
	...
	private static final String testCustomerId = "608252633";

	private UserInfo getTestUser() throws YsProException {
		String customerId = testCustomerId;
		UserInfo userInfo = api.findCustomerInfo(testCustomerId);
		Assert.assertNotNull(userInfo);
		Assert.assertNotNull(userInfo.getStatus());
		Assert.assertEquals(0, userInfo.getStatus().intValue());
		Assert.assertEquals("OK message expected", "OK", userInfo.getMessage());
		Assert.assertNotNull("json must exist", userInfo.getJsonSource());
		Assert.assertNotNull("json must exist", userInfo.printJson());
		Assert.assertEquals("customer must be", testCustomerId, userInfo.getCustomer());
		return userInfo;
	}

	@Test
	public void customerInfo_1() throws Exception {
		UserInfo userInfo = getTestUser();
//		Assert.assertTrue("This user has a credit card", userInfo.isDibs());	
		String userId = userInfo.getUserId();
		String customerId = userInfo.getCustomer();
		UserInfo userInfo1 = api.findUserInfo(userId);
		Assert.assertNotNull(userInfo);
		Assert.assertTrue("status is 0", userInfo.getStatus() == 0);
		Assert.assertEquals("OK message expected", "OK", userInfo.getMessage());
		Assert.assertNotNull("json must exist", userInfo.printJson());
		Assert.assertEquals("customer must be", customerId, userInfo1.getCustomer());
		Assert.assertEquals("Expected user as the query user", userId, userInfo.getUserId());
//		Assert.assertTrue("This user has a credit card", userInfo.isDibs());
	}

	final String pwd = "Cool=Bro12";

	@Test
	public void testSetPassword_fail() throws YsProException {
		ProStoreResponse res = api.setPassword("99339949293-NOT-EXIST", "Cool=Bro1");
		Assert.assertNotNull(res);
		Assert.assertNotNull(res.getStatus());
		Assert.assertEquals(72, res.getStatus().intValue());
	}

	@Test
	public void testSetPassword() throws YsProException {
		UserInfo ui = getTestUser();
		ProStoreResponse res = api.setPassword(ui.getUserId(), pwd);
		Assert.assertNotNull(res);
		Assert.assertNotNull(res.getStatus());
		Assert.assertEquals(0, res.getStatus().intValue());
	}

	@Test
	public void testLogin() throws YsProException, UnknownHostException {
		// this is dependent on the previous testSetPassword succeeding !
		UserInfo ui = getTestUser();
		String res = api.login(ui.getUserName(), pwd);
		Assert.assertNotNull(res);
		Assert.assertEquals(50, res.length());
		String fail = api.login(ui.getUserName(), pwd + "willfail");
	...
	public void testSetUser() throws YsProException {
		UserInfo ui = getTestUser();
		ProStoreResponse res = api.updateUserInfo(ui.getUserId(), ui.getUserName(), ui.getUserName(), ui.getEmail(), "30549420");
		Assert.assertNotNull(res);
		Assert.assertTrue(res.getJsonSource().has("Data"));
		Assert.assertEquals(ui.getUserName(), res.getData().get("UserLogin").getAsString());
	}

	@Test
	public void testGetmailInfo() throws YsProException {
		UserInfo ui = getTestUser();
		ProStoreResponse eng = api.findEngagementOnlyActive(ui.getCustomer());
		JsonPath jp = JsonPath.compile("$.Data[?(@.ProductName == 'Webmail')].Properties[?(@.DataName=='KPMNumber')].Value");
		net.minidev.json.JSONArray jsonProd = jp.read(eng.getJsonSource().toString());
		Assert.assertNotNull(jsonProd);
		Assert.assertTrue(1 == jsonProd.size() || 0 == jsonProd.size());
		if (jsonProd.size() == 0) {
			return;
		}
		final String kpm = jsonProd.get(0).toString();
		ProStoreResponse psr = api.findMailGetInfo(kpm);
		Assert.assertNotNull(psr);
		Assert.assertTrue(0 == psr.getStatus() || 999 == psr.getStatus()); // also accept if the TDC mail platform is down. Doh.
	}

	@Test
	public void testGetNewPasswordMethods() throws YsProException, UnknownHostException {
		UserInfo ui = getTestUser();
		ProStoreResponse e = api.getNewPasswordMethods(ui.getUserName(), InetAddress.getByName("4.4.4.4"));
		Assert.assertNotNull(e);
		Assert.assertEquals(0, (int) e.getStatus());
	}
