...
/**
 * Created by IntelliJ IDEA. User: m14857 Date: Oct 21, 2010 Time: 2:31:13
 * PM<br/> Data structure reference to YouSee Data Migration Requirements: 5.7.1
 * SMP Mobile Broadband Service Definition
 */
public class SMPMobileBroadbandDEF extends BasicUnit {
    ...
    /**
     * There is a bug in MOBB tech cartridge that makes this sub service without
     * correct tdc_subscriber_id <p> Calling this method causes verifies the TDC
     * subscriber It will cause update of service plan when TDC subscriber is
     * missing or wrong. It is advised to log warning in such cases. </p> <p>
     * Note that updating needs to be executed as SMP update, cannot be part of
     * SMP delete The problem is that "delete" cannot do this update
     * concurrently with deleting.
     *
     * Therefore only update will fix the value. A fix in Provisioning would be
     * to perform a MOBB_FIX_SUBSCRIBER command/action and nest a delete
     * command.
     *
     * This takes so much logic that it might be better to fix the root-cause of
     * the problem. The root cause is on SMP the flow for adding
     * mobile-broad-band can fail. When it fails - the following manual task -
     * fails to update "tdc_subscriber_id" because a flow flag is set to "false"
     * and not "true". It would cause more deep analysis of this SMP flow to
     * understand the impact. The case is spiced with situations where a MSISDN
     * is in a wrong state at the service-portal. </p>
     *
     ...
     * matter does not change anything. </p>
     */
