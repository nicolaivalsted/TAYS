...
public class BackupCase extends AbstractCase {
	 ...
	 * @param keepModel true to use the model from subscriber case, false to start a new model (default originally
	 * false)
	 */
...
//		def.modem_id.setValue(modemId == null ? null : modemId.getId());
	...
	private boolean buildOrderFromAction(BusinessPosition position, Action action) {
		...
		if (service != null) {
			 ...
			 * <p>
			 * Tests shows that marking elements for something the element already is results in no order line in Sigma.
			 * This might be use full when sending commands to Sigma - so sending too much does not really matter in
			 * Sigma !!
			 * </p>
			 *
			 */
