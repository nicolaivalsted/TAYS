...
/**
 * Created with IntelliJ IDEA. User: aka Date: 01/06/12 Time: 23.51 Read
 * subscription based on subscriber Id
 */
	...
	static class Parser extends ResponseParser<Response> {
		...
		public ResponseEntity parseEntityValue(EntityValue entVal) {
			...
			if (entVal instanceof SubContactType) { // service activation schema
				...
				SubSvcKeyType parentKey = subKey.getParentServiceKey(); // link
																		// back
																		// to
																		// parent
																		// service
