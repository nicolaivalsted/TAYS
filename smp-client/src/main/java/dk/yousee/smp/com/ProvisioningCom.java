...
/**
 * Created with IntelliJ IDEA. User: aka Date: 02/06/12 Time: 20.07
...
public class ProvisioningCom extends SmpCom<Order, ExecuteOrderReply> {
	...
	private static class Pack extends RequestPack<Order> {
		...
		public XmlObject createXml(Order order) {
			...
			execDoc = createXmlOrderDoc(order, orderId, order.getOnlySub());
			return execDoc;
		}
		 ...
		 *
		 * @param order
		 *            the order document from client
		 * @param orderId
		 *            ordernumber (always -1)
		 * @return xml document to be processed at Sigma
		 */
		private ExecuteOrderRequestDocument createXmlOrderDoc(Order order, Integer orderId, boolean subscriber) {
			OrderValue orderValue;
			orderValue = createForExistingCustomer(order, orderId, subscriber);
		...
		private OrderValue createForExistingCustomer(Order order, Integer orderId, boolean susbcriber) {
		 ...
		 *
		 * @param orderItemList
		 *            list to add plan to. Precondition: N elements,
		 *            Postcondition: N+1 elements
		 * @param servicePlan
		 *            client input to be converted to XML
		 */
		static void addServiceRequest(ActionOrderValue.OrderItemList orderItemList, OrderData servicePlan) {
		...
		private static void addServiceRequestForSampSub(ActionOrderValue.OrderItemList orderItemList, OrderData servicePlan) {
		...
		// todo: THIS MUST BE A BUG !!!!!!!
		private static void deleteSubscription(ActionOrderValue.OrderItemList orderItemList, OrderData servicePlan, String kundeId) {
