...
@Ignore
public class BackupCaseIT {
    private static final Logger logger = Logger.getLogger(BackupCaseIT.class);

    private BackupCase test;
...
//        connector.setSmpHost(SmpConnectorImpl.T_NET_SMP_HOST);
    ...
    public void step_02_readProvisioning() throws Exception {
        test = new BackupCase(orderService, acct);
        model = test.getModel();
        Assert.assertTrue("customer must exist", model.customerExists());
        List<BackupService> services = model.find().BackupService();
        ...
        BackupService service = services.get(0);
        BusinessPosition position = service.getPosition();

        BackupCase.BackupData data = test.readProvisioning(position);
    ...
    public void step_04_readAll() throws Exception {
        ...
        List<BackupCase.BackupActivationData> list = test.readAll();

        for (BackupCase.BackupActivationData one : list) {
            Assert.assertNotNull("BackupActivationData can never be null", one);
